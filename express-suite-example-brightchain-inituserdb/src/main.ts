...
import { Constants as AppConstants } from '@express-suite-example-brightchain/api-lib';
...
async function main() {
  ...
  const envDir = join(
    ...
    'express-suite-example-brightchain-inituserdb',
    '.env',
  );
  ...
  if (result.success && result.backend) {
    ...
    if (process.argv.includes('--setEnv')) {
      const apiEnvPath = join(
        BrightDbApplication.distDir,
        '..', // Go up from dist
        'express-suite-example-brightchain-api',
        '.env',
      );
      debugLog(
        env.detailedDebug,
